...
void fitCircle(double *data, double &error, cv::RotatedRect &res, double scale)
{
    ...
    res.size.width =  res.size.height = 1 / abs(_dls_X[0]) * scale;
    res.angle = 0;
...
void ElliFit(double *data, double &error, cv::RotatedRect &res, double scale)
{
	...
	res.size.width = rp[2] * scale;
	res.size.height = rp[3] * scale;
	res.angle = rp[4];
}
...
{
    cv::Mat S(dim, dim, CV_64FC1), C(dim, dim, CV_64FC1);
    memcpy(S.data, _dataS, sizeof(double) * dim * dim);
    memcpy(C.data, _dataC, sizeof(double) * dim * dim);

    cv::Mat dls_D, dls_V, S2, dls_D2, dls_V2, dls_X;
    *err = -3;

    cv::eigen(S, dls_D, dls_V);
    double *_dls_D = (double*)dls_D.data;

	if (_dls_D[dim - 1] > 1e-10)
	{
        for (int i = 0; i < dim; i++)
            dls_V.row(i) = dls_V.row(i) / sqrt(fabs(_dls_D[i]));
		S2 = dls_V*C*dls_V.t();
		cv::eigen(S2, dls_D2, dls_V2);
        double *_dls_D2 = (double*)dls_D2.data;
		if (_dls_D2[0] > 0)
		{
			*err = 1.0 / _dls_D2[0];
			dls_X = (dls_V2.row(0)*dls_V).t();
		}
		else
		{
			*err = -1;
	...
	{
		dls_X = dls_V.row(dim - 1).t();
		*err = 0;
	}
    memcpy(_dataX, dls_X.data, sizeof(double) * dim);
}

void MeasureCircleSampsonError(double *px, double *py, double *pz, int num, double *parms, double *error)
{
    *error = 0;
    cv::Matx14d parms2d(parms), ui;
    cv::Matx34d dui = cv::Matx34d::zeros();
    cv::Matx44d Aui, Bui;
    double k1, k2;
    for(int i = 0; i < num; i++)
    {
        double tpx = px[i], tpy = py[i], tpz = pz[i];

        ui.val[0] = tpx * tpx + tpy * tpy, ui.val[1] = 2 * tpx * tpz;
        ui.val[2] = 2 * tpy * tpz, ui.val[3] = tpz * tpz;

        dui.val[0] = 2 * tpx, dui.val[1] = 2 * tpz;
        dui.val[4] = 2 * tpy, dui.val[6] = 2 * tpz;
        dui.val[9] = 2 * tpx, dui.val[10] = 2 * tpy, dui.val[11] = 2 * tpz;

        Aui = ui.t() * ui;
        Bui = dui.t() * dui;

        k1 = (parms2d * Aui * parms2d.t()).val[0];
        k2 = (parms2d * Bui * parms2d.t()).val[0];
...
// def MeasureSampsonError(parms:np.ndarray, px: np.ndarray, py: np.ndarray, pz: np.ndarray):
    
//     err = 0
//     parms2d = np.array([parms])
//     for each_x, each_y, each_z in zip(px, py, pz):
//         ui = np.array([[each_x * each_x + each_y * each_y, 2 * each_x * each_z, 
//                        2 * each_y * each_z, each_z * each_z]])
//         dui = np.array([[2 * each_x, 2 * each_z, 0, 0],
//                         [2 * each_y, 0, 2 * each_z, 0],
//                         [0, 2 * each_x, 2 * each_y, 2 * each_z]])
//         Aui = np.matmul(ui.transpose(), ui)
//         Bui = np.matmul(dui.transpose(), dui)
        
//         # print(parms2d)
//         # print(Aui)
        
//         k1 = np.matmul(parms2d, np.matmul(Aui, parms2d.transpose()))[0, 0]
//         k2 = np.matmul(parms2d, np.matmul(Bui, parms2d.transpose()))[0, 0]
//         err += k1 / k2

...
from ElpPy.plane_fitting.plane_proc import fit_plane_bayes

from ElpPy.apriltags_rgbd import LM_minimize as lm
from ElpPy.apriltags_rgbd import rigid_transform as trans
...
def sample_depth_plane(depth_image, image_pts, K):
	...
	# 获取码标内部的点，得到N*3的点集samples_depth，cov是个N*1的定值
	xcoord = image_pts[:, 0]
	ycoord = image_pts[: ,1]
	...
	depth_plane_est = fit_plane_bayes(samples_depth, cov)
...
# 输入：彩色图像、深度图像、码标平面的4个点坐标，图像上顺序4个点坐标、相机内参3x3矩阵，一个5x1的全是0的矩阵
def solvePnP_RGBD(depth_image, object_pts, image_pts, K, D, verbose = 0):
	if (depth_image.ndim == 3):
		depth_image = depth_image[:,:,0]

	# 平面拟合，返回的是个平面类BayesPlane

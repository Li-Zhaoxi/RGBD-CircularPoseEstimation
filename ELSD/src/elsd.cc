...
static image_double cvMat2_image_double(cv::Mat &imgG)
{
  int ysize = imgG.rows;
  int xsize = imgG.cols;
  image_double image;
  ...
  for (y = 0; y < ysize; y++)
    for (x = 0; x < xsize; x++)
      image->data[x + y * xsize] = (double)imgG.at<uchar>(y, x);
  return image;
}
...
gauss_filter new_gauss_filter(unsigned int dim)
{
  ...
  if (dim <= 0)
    error("new_gauss_filter: invalid filter size");

  fil = (gauss_filter)malloc(sizeof(struct gauss_filter_s));
  if (fil == NULL)
    error("new_gauss_filter: not enough memory");
                      ...
                      image_double *grad, unsigned int n_bins, double max_grad)
{
  ...
  *mem_p = (void *)list;
  ...
  for (i = 0; i < ysize; i++)
    angles->data[i * xsize + xsize - 1] = NOTDEF;
  for (j = 0; j < xsize; j++)
    angles->data[(ysize - 1) * xsize + j] = NOTDEF;
...
void EllipseDetection(image_double image, double rho, double prec, double p,
                      ...
                      int *circ_count, int *line_count, char *fstr, std::vector<FullELlipse> &allElps)
{
  allElps.clear();
  ...
  std::string svg_file_path = std::string(fstr) + ".svg";
  svg = init_svg(svg_file_path.c_str(), xsz0, ysz0);
  ...
  std::string txt_file_path = std::string(fstr) + ".ellipses.txt";
  FILE *fe = fopen(txt_file_path.c_str(), "wt");
        ...
        angles->data[list_p->y * angles->xsize + list_p->x] != NOTDEF)
    {
      ...
      if (reg_size > 2)
      {
        ...
        if (nfa[2] > mlog10eps && nfa[2] > nfa[0] && nfa[2] > nfa[1] && regp_size[2] > min_size[2]) /* ellipse */
        {
          ...
          double _parame[5];
          int _pext[8];
          memcpy(_parame, parame, sizeof(double) * 5);
          memcpy(_pext, pext, sizeof(int) * 8);
          write_svg_ellipse(fe, svg, _parame, _pext, smooth);
          FullELlipse tmp;
          memcpy(_parame, parame, sizeof(double) * 5);
          memcpy(_pext, pext, sizeof(int) * 8);
          record_ellipse(_parame, _pext, smooth, rege, regp_size[2], tmp);
          allElps.push_back(tmp);
          isellipse = true;
          for (i = 0; i < regp_size[0]; i++)
        ...
        {
          // (nfa[1] > mlog10eps && nfa[1] > nfa[0] && regp_size[1] > min_size[1])
          // else if (nfa[1] > mlog10eps && nfa[1] > nfa[0] && nfa[1] > nfa[2] && regp_size[1] > min_size[1]) /* circle */
          (*circ_count)++;
          /*if (smooth)
          ...
          double _paramc[5];
          int _pext[8];
          memcpy(_paramc, paramc, sizeof(double) * 5);
          memcpy(_pext, pext, sizeof(int) * 8);
          write_svg_circle(fe, svg, _paramc, _pext, smooth);
          FullELlipse tmp;
          memcpy(_paramc, paramc, sizeof(double) * 5);
          memcpy(_pext, pext, sizeof(int) * 8);
          record_circle(_paramc, _pext, smooth, regc, regp_size[1], tmp);
          allElps.push_back(tmp);
...
void writeJson(std::string out_json, std::vector<FullELlipse> &allelps)
{
  int num_elps = allelps.size();

  json node_final;

  node_final["total"] = num_elps;
  for (int i = 0; i < num_elps; i++)
  {
    json tmpnode;

    json elpnode;
    elpnode["cx"] = allelps[i].elp.center.x;
    elpnode["cy"] = allelps[i].elp.center.y;
    elpnode["rx"] = allelps[i].elp.size.width;
    elpnode["ry"] = allelps[i].elp.size.height;
    elpnode["angle"] = allelps[i].elp.angle;
    tmpnode["elp"] = elpnode;

    std::vector<std::vector<int>> regs;
    auto &elp_regs = allelps[i].regs;
    for (int k = 0; k < elp_regs.size(); k++)
    {
      std::vector<int> tmp;
      tmp.push_back(elp_regs[k].x);
      tmp.push_back(elp_regs[k].y);
      regs.push_back(tmp);
    }
    tmpnode["regs"] = regs;

    tmpnode["start"] = allelps[i].angle_start;
    tmpnode["end"] = allelps[i].angle_end;
    tmpnode["isFull"] = allelps[i].isFull;
    tmpnode["fa"] = allelps[i].fa;
    tmpnode["fs"] = allelps[i].fs;

    std::vector<double> tmp1(2);
    tmp1[0] = allelps[i].pt1.x;
    tmp1[1] = allelps[i].pt1.y;
    tmpnode["pt1"] = tmp1;

    std::vector<double> tmp2(2);
    tmp2[0] = allelps[i].pt2.x;
    tmp2[1] = allelps[i].pt2.y;
    tmpnode["pt2"] = tmp2;

    char idx[32];
    sprintf(idx, "%d", i);

    node_final[std::string(idx)] = tmpnode;
  }

  std::ofstream(out_json) << node_final;
}

void drawFullEllipses(cv::Mat &imgC, std::vector<FullELlipse> &allelps)
{
  int num_elps = allelps.size();
  for (int i = 0; i < num_elps; i++)
  {
    auto &pts = allelps[i].regs;
    // 画点
    for (int k = 0; k < pts.size(); k++)
    {
      cv::circle(imgC, cv::Point(pts[k].x + 0.5, pts[k].y + 0.5), 1, cv::Scalar(255, 0, 0), 1);
    }
    // 画椭圆
    if (allelps[i].isFull)
    {
      cv::RotatedRect tmp = allelps[i].elp;

      tmp.size.width *= 2;
      tmp.size.height *= 2;
      tmp.angle = tmp.angle / CV_PI * 180.0;
      cv::ellipse(imgC, tmp, cv::Scalar(0, 0, 255), 2);
    ...
    else
    {
      ...
      // tmp.size.height *= 2;
      tmp.angle = tmp.angle / CV_PI * 180.0;
      double angle_start = allelps[i].angle_start;
      double angle_end = allelps[i].angle_end;

      angle_start = angle_start / CV_PI * 180.0;
      angle_end = angle_end / CV_PI * 180.0;

      // std::cout << tmp.center << tmp.size << tmp.angle << std::endl;
      // std::cout << angle_start << ", " << angle_end << std::endl;

      cv::circle(imgC, cv::Point(allelps[i].pt1.x + 0.5, allelps[i].pt1.y + 0.5), 2, cv::Scalar(0, 0, 255), 2);
      cv::circle(imgC, cv::Point(allelps[i].pt2.x + 0.5, allelps[i].pt2.y + 0.5), 2, cv::Scalar(0, 0, 255), 2);

      // cv::ellipse(imgC, tmp.center, tmp.size, tmp.angle, angle_start, angle_end, cv::Scalar(0, 0, 255), 2);
    }
    // auto& elp = allelps[i].elp;
    // cv::ellipse(imgC, elp.center, elp.size, elp.angle, allelps[i].angle_start, allelps[i].angle_end)
  }
}
...
int main(int argc, char **argv)
{
  ...
  cv::Mat imgC = cv::imread(imgpath), imgG;
  if (imgC.channels() == 3)
  {
  ...
  image = cvMat2_image_double(imgG);
  std::vector<FullELlipse> allElps;

  int low = rho, high = 2 * rho;
  calCannyThreshold(imgG, low, high);
  // void calCannyThreshold(cv::Mat &ImgG, int &low, int &high)

  EllipseDetection(image, low, prec, p, eps, smooth, &ell_count, &circ_count,
                   &line_count, argv[1], allElps);
  // printf("%s\n", argv[1]);
  // printf("%d elliptical arcs, %d circular arcs, %d line segments\n",
  //        ell_count, circ_count, line_count);

  std::string out_json = imgpath + ".json";
  writeJson(out_json, allElps);

  // drawFullEllipses(imgC, allElps);
  // cv::imshow("res", imgC);
  // cv::waitKey();
  return 0;
}

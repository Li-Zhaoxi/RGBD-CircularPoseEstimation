...
/*----------------------------------------------------------------------------*/
FILE *init_svg(const char *filename, unsigned int xsize, unsigned int ysize)
{
  FILE *svg;
  ...
  if (svg == NULL)
    error("Error: unable to open SVG output file.");
...
void record_circle(double *param, int *pext, int smooth, struct point3 *reg, int reg_size, FullELlipse &full_elp)
{
  double scale = 1;
  if (smooth)
    scale = 1.25;
  full_elp.regs.resize(reg_size);
  for(int i = 0; i < reg_size; i++)
  {
    full_elp.regs[i].x = reg[i].x * scale;
    full_elp.regs[i].y = reg[i].y * scale;
  }
  ...
  {
    full_elp.elp.center.x = param[0];
    full_elp.elp.center.y = param[1];
    full_elp.elp.angle = 0;
    full_elp.elp.size.width = param[2];
    full_elp.elp.size.height = param[2];
    full_elp.isFull = 1;
  }
  else
  {
    ...
    full_elp.fa = fa, full_elp.fs = fs;
    full_elp.pt1.x = x1, full_elp.pt1.y = y1;
    full_elp.pt2.x = x2, full_elp.pt2.y = y2;

    full_elp.angle_start = ang_start;
    full_elp.angle_end = ang_end;
  }
}
...
void record_ellipse(double *param, int *pext, int smooth, struct point3 *reg, int reg_size, FullELlipse &full_elp)
{
  double scale = 1;
  if (smooth)
    scale = 1.25;
  ...
  if (angle_diff(ang_start, ang_end) < M_2__PI * SQRT2 / C && angle_diff_signed(ang_start, ang_end) > 0)
  {
    full_elp.elp.center.x = param[0];
    full_elp.elp.center.y = param[1];
    full_elp.elp.angle = (param[4]) * 180 / M_PI;
    full_elp.elp.size.width = param[2];
    full_elp.elp.size.height = param[3];
    full_elp.isFull = 1;
  }
  else
  {

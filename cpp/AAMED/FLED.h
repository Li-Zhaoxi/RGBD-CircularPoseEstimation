...
public:
	...
	vector<std::vector<int>> arcsIdxConductEllipse;
	~FLED()
	{
		release();
	}
...
public:// Draw Data and Write Information Functions
	...
	void getEllipseEdgePoints(int elpidx, std::vector<cv::Point>& pts);
	void drawEllipseEdgePoints(Mat ImgG, int idx);
public: 
...
public: // The public functions that used for Python.
	...
	int getEllipseEdgePointsNum(int elpidx)
	{
		std::vector<int> arcIdx = arcsIdxConductEllipse[elpidx];
		int tmp = 0;
		for (int i = 0; i < arcIdx.size(); i++)
		{
			cv::Point3i info = Arc2Edge[arcIdx[i]];
			int edge_idx = info.x, edge_st = info.y, edge_ed = info.z;
			tmp += edge_ed - edge_st + 1;
		}
		return tmp;
	}
	void getEllipseEdgePointsPy(int elpidx, unsigned int *_pts)
	{
		std::vector<cv::Point> pts;
		getEllipseEdgePoints(elpidx, pts);
...
protected:
	...
	void FSA_Segment(vector<Point>& dpContour, int dpIdx);
...
private:
	...
	std::unordered_map<int, int> DP2Edge;
	std::vector<std::vector<int>> DPIdx;
	std::vector<int> idxTmp, sortedIdx;
	std::unordered_map<int, cv::Point3i> Arc2Edge;
protected:

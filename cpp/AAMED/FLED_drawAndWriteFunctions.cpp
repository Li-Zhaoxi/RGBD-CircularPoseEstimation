...
void FLED::getEllipseEdgePoints(int elpidx, std::vector<cv::Point>& pts)
{
	...
	for (int i = 0; i < arcIdx.size(); i++)
	{
		...
		pts.insert(pts.end(), edgeContours[edge_idx].begin() + edge_st, edgeContours[edge_idx].begin() + edge_ed + 1);
	}

}

void FLED::drawEllipseEdgePoints(Mat ImgG, int idx)
{
	std::vector<cv::Point> pts;
	getEllipseEdgePoints(idx, pts);
	Mat Img_T = ImgG.clone();
	cvtColor(Img_T, Img_T, CV_GRAY2BGR);
	for (int i = 0; i < pts.size(); i++)
	{
		cv::circle(Img_T, cv::Point(pts[i].y, pts[i].x), 2, cv::Scalar(0, 0, 255));
	}
	cv::imshow("EllipsePoints", Img_T);
}
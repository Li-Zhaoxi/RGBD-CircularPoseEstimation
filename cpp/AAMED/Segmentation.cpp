...
void FLED::FSA_Segment(vector<Point>& dpContour, int dpidx)
{
	...
	for (int i = 1; i < dpContourNum; i++)
	{
		...
		int edge_st_idx = DPIdx[dpidx][arc_st], edeg_ed_idx = DPIdx[dpidx][arc_ed];

		FSA_ArcContours.push_back(oneContour);
		Arc2Edge.insert(std::pair<int, cv::Point3i>(FSA_ArcContours.size() - 1, cv::Point3i(DP2Edge[dpidx], edge_st_idx, edeg_ed_idx)));
	}
}
...
void FLED::SortArcs(vector<vector<Point>>& detArcs)
{
	...
	sort(sortedIdx.begin(), sortedIdx.end(),
		[&_tmpdata, &tmpdCols, &detArcs](int i1, int i2)
		{
			Point st, ed;
			st = detArcs[i1][0], ed = detArcs[i1].back();
			int arc1_edge_num = _tmpdata[ed.x * tmpdCols + ed.y].edgeID - _tmpdata[st.x * tmpdCols + st.y].edgeID;

			st = detArcs[i2][0], ed = detArcs[i2].back();
